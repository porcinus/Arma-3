...
_classParams = _this param [0,[],[configfile,[]]];
_display = _this param [1,displaynull,[displaynull]];
...
_mousepos = _this param [2,[0.5,0.5],[[]]];
...
_text = _classParams param ["text" call bis_fnc_ORBATGetGroupParams,"",[""]];
_picture = _classParams param ["insignia" call bis_fnc_ORBATGetGroupParams,"",[""]];
_infoText = _classParams param ["infoText" call bis_fnc_ORBATGetGroupParams,"",[""]];
_assets = _classParams param ["assetsText" call bis_fnc_ORBATGetGroupParams,"",[""]];

//--- Set text so height can be calculated
_tooltipInfoBackground ctrlsetstructuredtext parsetext ("<t color='#00000000' shadow='0'>" + _text + "</t>");
...
_tooltip ctrlsetposition [
	...
	_infoPosY,
	_infoPosW,
	if ((_infoPosH + _assetsH + _pictureH) > _marginTop) then {_infoPosH + (_marginTop * 3 + _assetsH) + (_marginTop + _pictureH + (_infoPosW * ((1 - _tooltipPictureCoef) / 2)) * 4/3)} else {0}
];
