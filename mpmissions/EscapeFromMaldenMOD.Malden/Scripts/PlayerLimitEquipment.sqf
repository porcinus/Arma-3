...
if (BIS_loadoutLevel > 1) then { //remove common equipment between level 2 and 3
	if !(typeOf player == "O_Soldier_SL_F") then {player removePrimaryWeaponItem "optic_Arco_blk_F"}; //if not leader, remove ARCO
	if (count (backpackItems player) == 0) then {removeBackpack player;}; //remove backpack if empty
	player unassignItem "acc_pointer_IR"; player removeItem "acc_pointer_IR"; //remove IR pointer
...
if (BIS_loadoutLevel == 3) then { //remove remaining optics if level 3
	...
	player removePrimaryWeaponItem "optic_Arco_blk_F";
};
