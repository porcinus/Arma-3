...
switch (toLower _action) do {
	...
	case (toLower "onEachFrame") : {
		...
		//Go through all items in queue
		{
			...
			if (typeName _p == typeName [] && { count _p >= 9 }) then
			{
				_p set [8, _forEachIndex];
			};

			//Execute item
			["itemExecute", _p] call BIS_fnc_loop;
	...
	case (toLower "itemExecute") : {
		...
		_id 			= _parameters param [0, "", [""]];
		_code 			= _parameters param [1, {}, [{}, ""]];
		_timer			= _parameters param [2, 0, [0], 2];
		_timerType 		= _parameters param [3, "seconds", [""]];
		_conditionStart		= _parameters param [4, { true }, [{}]];
		_conditionEnd		= _parameters param [5, { false }, [{}]];
		_executeOnce 		= _parameters param [6, false, [false]];
		_lastExecutionTime 	= _parameters param [7, time, [0]];
		_index			= _parameters param [8, 0, [0]];
	...
	case (toLower "itemAdd") : {
		...
		_id 			= _parameters param [0, "", [""]];
		_code 			= _parameters param [1, {}, [{}, ""]];
		_timer			= _parameters param [2, 0, [0]];
		_timerType 		= _parameters param [3, "seconds", [""]];
		_conditionStart		= _parameters param [4, { true }, [{}]];
		_conditionEnd		= _parameters param [5, { false }, [{}]];
		_executeOnce 		= _parameters param [6, false, [false]];
