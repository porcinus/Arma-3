...
//if (bis_genericSentenceMode > 0) then {
	...
	if (_sentence != "") then {
		...
		if (
			...
			private ["_canRepeat"];
			_canRepeat = _this param [3,false,[false]];
			...
			if (_sentence != bis_genericSentenceLast || _canRepeat) then {
				...
				//--- Get speaker
				_speaker = _this param [1,bis_genericSentenceSpeaker,[objnull,{}]];
				if (typename _speaker == typename {}) then {
					_speaker = (call _speaker) param [0,objnull,[objnull]];
				};
