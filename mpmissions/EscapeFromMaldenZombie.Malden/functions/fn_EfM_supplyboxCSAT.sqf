...
Custom content for supplybox
Modified for CSAT side, ammo for Navid and Zubr added
*/
...
// Check for validity
if (isNull _box) exitWith {[format["BIS_fnc_EfM_ammoboxCSAT : Non-existing unit box %1 used!",_box]] call BIS_fnc_NNS_debugOutput;};
...
if (random 100 < 65) then {
	_box addMagazineCargoGlobal ["NLAW_F",2 + (round random 2)]; _box addMagazineCargoGlobal ["NLAW_F",2 + (round random 2)]; _box addWeaponCargoGlobal ["launch_NLAW_F",1];
} else {
...
// RPG-42
/*
_box addMagazineCargoGlobal ["NLAW_F",2 + (round random 2)];
_box addMagazineCargoGlobal ["NLAW_F",2 + (round random 2)];
...
// Titan AT
_box addMagazineCargoGlobal ["Titan_AT",2 + (round random 3)]; //NNS
_box addWeaponCargoGlobal ["launch_O_Titan_short_F",1]; //NNS
