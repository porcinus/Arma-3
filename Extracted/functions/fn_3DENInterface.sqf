...
switch (tolower _mode) do {
	case "showinterface": {
		...
		private _show = param [1,round ctrlfade (_display displayctrl IDC_DISPLAY3DEN_PLAY) > 0,[false]]; //param [1,round ctrlfade (_display displayctrl IDC_DISPLAY3DEN_PLAY) > 0];
		private _fade = [1,0] select _show; //(1 - round ctrlfade (_display displayctrl IDC_DISPLAY3DEN_PLAY));
		{
		...
		"mode" call bis_fnc_3DENInterface;
		"submode" call bis_fnc_3DENInterface;
	};
	...
	case "showpanelright": {
		...
		private _ctrl = _display displayctrl (_data select 0);
		private _pos = ctrlposition _ctrl;
		private _posY = _pos select 1;
		private _show = param [1,_posY > 5,[false]]; //param [1,_posY > 5];
		if (_show) then {
		...
		private _ctrlScrollLock = _display displayctrl (_data select 1);
		_ctrlScrollLock ctrlsetposition _pos;
		_ctrlScrollLock ctrlcommit 0;

		private _ctrlButtonToggle = _display displayctrl (_data select 2);
		_ctrlButtonToggle ctrlenable !_show;

		//--- Adjust navigation widget
		if (_data select 4) then {
			private _ctrlNav = _display displayctrl IDC_DISPLAY3DEN_NAVIGATION_WIDGET;
			private _ctrlNavPos = ctrlposition _ctrlNav;
			private _offset = (_ctrlNavPos select 2) * 0.2;
	...
	case "tableft": {
		private _display = finddisplay IDD_DISPLAY3DEN;
		private _id = param [1,0,[0]];
		["ShowPanelLeft",true] call bis_fnc_3DENINterface;
		private _ctrlTabs = _display displayctrl IDC_DISPLAY3DEN_TABLEFT_SECTIONS;
		for "_i" from 0 to 10 do {
			private _idc = _ctrlTabs lbvalue _i;
			if (_idc <= 0) exitwith {};
			private _ctrl = _display displayctrl _idc;
			_ctrl ctrlshow (_i == _id);
		};
	};
	case "tabright": {
		private _display = finddisplay IDD_DISPLAY3DEN;
		...
		private _ctrlTabs = _display displayctrl IDC_DISPLAY3DEN_TABRIGHT_SECTIONS;
		for "_i" from 0 to 10 do {
			private _idc = _ctrlTabs lbvalue _i;
			if (_idc <= 0) exitwith {};
			private _ctrl = _display displayctrl _idc;
		...
		if (_id == 0) then {
			...
			} foreach ["SelectSubmode1","SelectSubmode2","SelectSubmode3","SelectSubmode4","SelectSubmode5","SelectSubmode6"];
		};
	};
	case "buttonplay": {
		private _ctrlPlay = _this param [1,finddisplay IDD_DISPLAY3DEN displayctrl IDC_DISPLAY3DEN_PLAY,[controlnull]];
		private _isMission = false;
		private _enable = true;

		private _textSection = switch 1 do {
		...
		private _textMP = if (is3DENMultiplayer) then {
			_enable = _isMission;
	...
	case "mode": {
		...
		private _ctrlModeLabels = _display displayctrl IDC_DISPLAY3DEN_MODELABELS;
		_ctrlModeLabels lbsetcursel _id;
	};
	case "submode": {
		private _display = finddisplay IDD_DISPLAY3DEN;
		private _ctrlWarning = _display displayctrl IDC_DISPLAY3DEN_CREATE_OBJECT_EMPTY_WARNING;

		//--- Vehicle placement control panel
		private _ctrlVehicleGroup = _display displayctrl  IDC_DISPLAY3DEN_VEHICLE_CONTROLPANEL;
		_ctrlVehicleGroup ctrlshow (get3denactionstate "SelectObjectMode" > 0 && get3denactionstate "SelectSubmode5" == 0);

		//--- Show custom compositions control panel
		private _ctrlCustomCompositionGroup = _display displayctrl IDC_DISPLAY3DEN_CUSTOMCOMPOSITION_CONTROLPANEL;
		_ctrlCustomCompositionGroup ctrlshow (get3denactionstate "SelectGroupMode" > 0 && get3denactionstate "SelectSubmode6" > 0);
	};
	case "placemode": {
		private _display = finddisplay IDD_DISPLAY3DENPLACE;
		private _ctrlMode = _display displayctrl IDC_DISPLAY3DEN_MODES;
		private _ctrlModeLabels = _display displayctrl IDC_DISPLAY3DEN_MODELABELS;
		_ctrlModeLabels lbsetcursel lbcursel _ctrlMode;
	};
	case "placesubmode": {
		private _display = finddisplay IDD_DISPLAY3DENPLACE;
		private _ctrlMode = _display displayctrl IDC_DISPLAY3DEN_MODES;
		private _ctrlSubmode = _display displayctrl IDC_DISPLAY3DEN_SUBMODES;

		//--- Vehicle placement control panel
		private _ctrlVehicleGroup = _display displayctrl  IDC_DISPLAY3DEN_VEHICLE_CONTROLPANEL;
		_ctrlVehicleGroup ctrlshow (lbcursel _ctrlMode == 0 && lbcursel _ctrlSubmode != 4);
	};
	case "expandassetbrowser";
	case "expandentitylist": {
		private _idcs = if (_mode == "expandentitylist") then {[IDC_DISPLAY3DEN_EDIT]} else {[IDC_DISPLAY3DEN_CREATE_ALL]};
		private _fnc_expand = {
			private _path = _this;
			_ctrlTree tvexpand _path;
			for "_i" from 0 to (_ctrlTree tvcount _path) do {
				(_path + [_i]) call _fnc_expand;
			};
		};
		private _display = if !(isnull finddisplay IDD_DISPLAY3DENPLACE) then {finddisplay IDD_DISPLAY3DENPLACE} else {finddisplay IDD_DISPLAY3DEN};
		{
			private _ctrlTree = _display displayctrl _x;
			if (ctrlenabled _ctrlTree) then {[] call _fnc_expand;};
		} foreach _idcs;
	};
	case "collapseassetbrowser";
	case "collapseentitylist": {

		private _idcs = if (_mode == "collapseentitylist") then {[IDC_DISPLAY3DEN_EDIT]} else {[IDC_DISPLAY3DEN_CREATE_ALL]};
		private _fnc_collapse = {
			private _path = _this;
			if (count _path > 0) then {_ctrlTree tvcollapse _path;};
			for "_i" from 0 to (_ctrlTree tvcount _path) do {
		...
		{
			private _ctrlTree = _display displayctrl _x;
			if (ctrlenabled _ctrlTree) then {[] call _fnc_collapse;};
		} foreach _idcs;
	...
	case "navigationwidget": {
		private _display = finddisplay IDD_DISPLAY3DEN;
		private _ctrlNav = _display displayctrl IDC_DISPLAY3DEN_NAVIGATION_WIDGET;
		private _show = _this param [1,!ctrlshown _ctrlNav,[true]];
	...
	case "onterrainnew": {
		if ("Preferences" get3DENMissionAttribute "startMap") then {
			//--- Show map and show the whole terrain
			if (get3DENActionState "togglemap" == 0) then {do3denaction "togglemap";};
			private _display = finddisplay IDD_DISPLAY3DEN;
			private _ctrlMap = _display displayctrl IDC_DISPLAY3DEN_MAP;
	...
	case "warningemptyvehicles": {
		...
		saveprofilenamespace;
		private _display = finddisplay IDD_DISPLAY3DEN;
		private _ctrlList = _display displayctrl IDC_DISPLAY3DEN_CREATE_OBJECT_EMPTY;
		private _ctrlListRef = _display displayctrl IDC_DISPLAY3DEN_CREATE_OBJECT_WEST;
		private _ctrlWarning = _display displayctrl IDC_DISPLAY3DEN_CREATE_OBJECT_EMPTY_WARNING;
	...
	case "placeempty": {
		{
			...
			private _ctrlToggleEmpty = _display displayctrl IDC_DISPLAY3DEN_VEHICLE_TOGGLE;
			_ctrlToggleEmpty cbsetchecked (get3denactionstate 'TogglePlaceEmptyVehicle' == 0);
		} foreach [IDD_DISPLAY3DEN,IDD_DISPLAY3DENPLACE];
	};
	case "showshortcut": {
		if (isnil {uinamespace getvariable "bis_fnc_3DENInterface_shortcuts"}) then {

			//--- Add keyboard shortcuts to all elements
			private _controlData = _this param [2,[],[[]]];
			private _control = _controlData param [0,controlnull,[controlnull]];
			private _display = ctrlparent _control;//finddisplay IDD_DISPLAY3DEN;
			private _ctrlMenuBar = _display displayctrl IDC_DISPLAY3DEN_MENUSTRIP;
			private _shortcuts = [];

			//--- Index shortcuts for each menu data
			private _fnc_menuBarExplore = {
				for "_i" from 0 to ((_ctrlMenuBar menuSize _this) - 1) do {
					private _path = _this + [_i];
					private _shortcutText = _ctrlMenuBar menushortcuttext _path;
		...
		private _data = _this param [1,"",[""]];
		private _index = _shortcuts find (tolower _data);
		if (_index >= 0) then {
			_controlData = _this param [2,[],[[]]];
			_control = _controlData param [0,controlnull,[controlnull]];
			private _config = _controlData param [1,configfile,[configfile]];
	...
	case "enablelayer": {
		...
		{
			_status = _status || !((_x get3DENAttribute "Transformation") select 0);
		} foreach (get3DENSelected "layer");
		{
			_x set3DENAttribute ["Transformation",_status];
		} foreach (get3DENSelected "layer");
	};
	case "showlayer": {
		private _status = false;
		{
			_status = _status || !((_x get3DENAttribute "Visibility") select 0);
		} foreach (get3DENSelected "layer");
		{
			_x set3DENAttribute ["Visibility",_status];
		} foreach (get3DENSelected "layer");
	};
	case "fadein": {
		disableserialization;
		private _display = finddisplay IDD_DISPLAY3DEN;
		private _ctrlBlack = _display displayctrl IDC_DISPLAY3DEN_BLACK;
