...
// Exceptions for air vehicles which are using those units as pilots
_exclude = ["C_man_sport_1_F_tanoan","C_man_1_1_F","I_soldier_F"];
{
	...
	if (_scope > 1) then {
		...
		if (_simulation in ["helicopter","helicopterx","helicopterrtd","airplane","airplanex"]) then {
			...
			if (!(_crew in _pilots) && !(_crew in _exclude)) then {_pilots set [count _pilots,_crew];};
		};
	};
...
endLoadingScreen;
hint "Finished diagMacrosNameSound";
_text