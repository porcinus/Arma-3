...
if !(_head isEqualTo false) then
{
	...
	if (_head isEqualType [] && {!_skipRandomization}) then
	{
		...
		else
		{
			...
			_headgearToUse = if (count _headgearPool > 0) then {selectRandom _headgearPool} else {""};
		};
	}
	else
	{
		if (_head isEqualType "") then
		{
			...
			if !(_cfgRoot isEqualTo NO_HEADGEAR) exitWith
			{
...
if !(_face isEqualTo false && {_unitType isKindOf "C_man_1"}) then
{
	...
	_faceToUse = "";
	if (_face isEqualType [] && {!_skipRandomization}) then
	{
		...
		{
			_faceToUse = if (count _faceList > 0) then {selectRandom _faceList} else {""};
		};
	}
	else
	{
