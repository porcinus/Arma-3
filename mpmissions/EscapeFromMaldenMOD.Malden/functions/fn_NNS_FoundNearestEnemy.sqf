...
Example: _tmp_nearest_enemy = [player,800] call BIS_fnc_NNS_FoundNearestEnemy;
...
if (_unit isEqualTo objNull) exitWith {["BIS_fnc_NNS_FoundNearestEnemy : Object needed"] call BIS_fnc_NNS_debugOutput;};
...
if((count _tmp_nearest_enemy) == 0) exitWith {
	[format["BIS_fnc_NNS_FoundNearestEnemy : unit:%1, Failed to detect enemy",_unit]] call BIS_fnc_NNS_debugOutput; //debug
	objNull
...
[format["BIS_fnc_NNS_FoundNearestEnemy : unit:%1, nearest enemy at %2m",_unit,_unit distance2d (_nearest_enemy select 0)]] call BIS_fnc_NNS_debugOutput; //debug
