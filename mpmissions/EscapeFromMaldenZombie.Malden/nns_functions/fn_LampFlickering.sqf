...
Example: 
_null = [lampobj,"mysound",100] call NNS_fnc_LampFlickering;
...
// Check for validity
if (isNull _lamp) exitWith {[format["fn_NNS_LampFlickering : Non-existing object %1 used!",_lamp]] call NNS_fnc_debugOutput;};
if (_maxDist < 0) exitWith {["fn_NNS_LampFlickering : Max distance < 0"] call NNS_fnc_debugOutput;};
if !(alive _lamp) exitWith {[format["fn_NNS_LampFlickering : object %1 is destroyed",_lamp]] call NNS_fnc_debugOutput;};
