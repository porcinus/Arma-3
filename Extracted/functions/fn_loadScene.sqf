...
_function = {
	...
	// Create vehicles
	{
		...
		if ((_pos select 2) > 1) then {_special = "FLY"};
		
		// Create vehicle
		private _pos = [10, 10, 6000 + (50 * _forEachIndex)];
		private _vehicle = createVehicle [_class, _pos, [], 0, _special];
	...
	// Process groups
	{
		...
		
		{
			...
			// Create unit
			private ["_unit"];
			_unit = _group createUnit [_class, [10,10,(5000 + (10 * _forEachIndex))], [], 0, "NONE"];
		...
		// Set as the correct variable
		if (_groupName != "") then {missionNamespace setVariable [_groupName, _group]};
	...
	// Process vehicles
	{
		private ["_name", "_pos", "_dir", "_pitchBank", "_damage", "_fuel", "_init"];
		_name = _x select 0;
		_pos = _x select 2;
		_dir = _x select 3;
		_pitchBank = _x select 4;
