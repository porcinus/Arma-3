...
private _shipPartInterrior = param [0, objNull];
private _animationState = param [1, 0];
if (!isNull _shipPartInterrior) then
{
	private _shipPart = ((nearestObjects [_shipPartInterrior,["Land_Destroyer_01_hull_04_F"],50, false]) param [0, objNull]);
	if (!isNull _shipPart) then
	{
		[_shipPart, _animationState, false] spawn bis_fnc_destroyer01AnimateHangarDoors;
	};
};
