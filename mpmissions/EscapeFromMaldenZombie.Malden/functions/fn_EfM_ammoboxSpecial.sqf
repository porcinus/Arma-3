...
// Check for validity
if (isNull _box) exitWith {[format["BIS_fnc_EfM_ammoboxSpecial : Non-existing unit box %1 used!",_box]] call BIS_fnc_NNS_debugOutput;};
...
_box addItemCargoGlobal ["optic_Nightstalker",2 + (round random 1)];
...
_box addWeaponCargoGlobal ["launch_O_Titan_F",1]; //NNS
...
_box addMagazineCargoGlobal ["Titan_AA",2 + (round random 2)]; //NNS
_box addMagazineCargoGlobal ["Titan_AT",2 + (round random 2)]; //NNS
