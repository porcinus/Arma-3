...
_input = _this param [0,objnull,[objnull,[],missionnamespace,sideunknown,grpnull,objnull,true]];
...
if (count _this > 3) exitwith {
	...
	if !(alive _unit) then {
		...
		//--- Respawn
		_oldUnit = _this param [1,0,[objnull,0]];
...
switch (typename _input) do {
	...
	case (typename true): {
		...
		_target = _input;
		_value = round (_this param [1,0,[0]]);
		_autodetect = _this param [2,false,[false]];
		...
		if (_autodetect) exitwith {
			_target = _this param [0,player,[objnull,true]];
		...
		if (_value != 0) then {
			...
			if (_tickets == 0) then {
				[[true,"respawnTicketsExhausted",[_target]],"bis_fnc_callscriptedeventhandler",false] call bis_fnc_mp;
			};
		};
