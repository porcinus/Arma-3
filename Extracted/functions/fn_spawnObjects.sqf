...
_count 		= _this param [2,1,[123]];
_offsetMatrix 	= _this param [3,[0,0,0],[[]]];
_offsetDir 	= _this param [4,0,[123]];
_dirNoise 	= _this param [5,{0},[{}]];
_enableSim	= _this param [6,false,[true]];
...
{
	_anchorObject 	 = _position param [0,objNull,[objNull]];
	_anchorPlacement = _position param [1,"TOP",[""]];
...
private["_objHeight","_x","_y","_z","_obj","_pos","_dirDelta","_objPrev"];
...
for "_i" from 0 to (_count-1) do
{
	...
	_obj enableSimulation _enableSim;
	...
	//disable collisions with previously created object
	else
	{
	    _objPrev = _objects param [_i - 1,objNull];

	    _obj disableCollisionWith _objPrev;
	    _objPrev disableCollisionWith _obj;
	};
