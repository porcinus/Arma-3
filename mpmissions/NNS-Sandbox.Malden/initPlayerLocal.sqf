...
if !(getMarkerColor "marker_respawn" == "") then {[player, "marker_respawn"] call BIS_fnc_moveToRespawnPosition;
} else {[player, leader group player] call BIS_fnc_moveToRespawnPosition}; //initial respawn don't exist, spawn on group leader
...
_null = player createDiaryRecord ["Diary", [localize "STR_NNS_briefing_title", format ["%1<br/><br/>%2<br/><br/>%3<br/><br/>%4<br/><br/>%5<br/><br/>%6",localize "STR_NNS_briefing_0",localize "STR_NNS_briefing_1",localize "STR_NNS_briefing_2",localize "STR_NNS_briefing_3",localize "STR_NNS_briefing_4",localize "STR_NNS_briefing_5"]]];
...
if (BIS_loadoutLevel == 0) then { //over-equiped
	{[player,_x] call BIS_fnc_addRespawninventory} forEach BIS_grpMainUnits; //add all respawn classes
	...
	[] spawn {
		while {sleep 10; true} do {
			...
			if ((rating player < -2000) && {vehicle player == player}) then { //renegade and not in vehicle
				if (([BIS_playerSide] call BIS_fnc_respawnTickets) != -1) then {[BIS_playerSide,1] call BIS_fnc_respawnTickets;}; //team based tickets, increse tickets by one to not punish whole team
