...
if (_trigger isEqualTo objNull) exitWith {["ObjectGroupMover.sqf : _trigger not exists"] call BIS_fnc_NNS_debugOutput;};
if (count _newPosArray < 1) exitWith {["ObjectGroupMover.sqf : _newPosArray is empty"] call BIS_fnc_NNS_debugOutput;};
...
if ((_randPos distance2D _startPos) < 0.1 && abs((_randPos select 2) - _startDir) < 0.1)  exitWith {["ObjectGroupMover.sqf : No move/rotation to do"] call BIS_fnc_NNS_debugOutput;};
...
[format["ObjectGroupMover.sqf : Objects around '%1' radius:%2m moved from [%3, %4] to [%5, %6] (%7m)",_trigger,_radius,_startPos select 0,_startPos select 1,_randPos select 0,_randPos select 1,(_randPos distance2D _startPos)]] call BIS_fnc_NNS_debugOutput; //debug
...
_checkpoint04 = [_trigger] execVM "Scripts\PopulateCheckpoint04.sqf";
