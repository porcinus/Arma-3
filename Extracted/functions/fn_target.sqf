...
switch (toLower _action) do {
	case (toLower "initialize") : {
		// Parameters
		private ["_object"];
		_object = _parameters param [0, objNull, [objNull]];
	...
	case (toLower "animate") : {
		...
		_object		= _parameters param [0, objNull, [objNull]];
		_direction	= _parameters param [1, "down", [""]];
	...
	case (toLower "hit") : {
		...
		_object 	= _parameters param [0, objNull, [objNull]];
		_shooter	= _parameters param [1, objNull, [objNull]];
		_damage		= _parameters param [2, 0, [0]];
		...
		if (["getAnimationPhase", [_object]] call BIS_fnc_target == UP) then {
			...
			if (_targetPopup == TRUE) then {
				...
				[_object, _targetPopupDelay] spawn {
					...
					_targetPopupDelay	= _this param [1, 1, [0]];
	...
	case (toLower "hitPart") : {
		// Parameters
		private ["_parts"];
		_parts = _parameters param [0, [], [[]]];
		
		// Parameters
		private ["_object", "_shooter"];
		_object 	= _parts param [0, objNull, [objNull]];
		_shooter	= _parts param [1, objNull, [objNull]];
	...
	case (toLower "getShooterDataIndex") : {
		...
		_object		= _parameters param [0, objNull, [objNull]];
		_shooterUid 	= _parameters param [1, "", [""]];
	...
	case (toLower "setShooterData") : {
		...
		_object			= _parameters param [0, objNull, [objNull]];
		_shooterUid 	= _parameters param [1, "", [""]];
		_shooterData	= _parameters param [2, [], [[]]];
	...
	case (toLower "getShotsDataWithFilter") : {
		...
		_object		= _parameters param [0, objNull, [objNull]];
		_shooterUid	= _parameters param [1, "", [""]];
		_by		= _parameters param [2, "", [""]];
		_code		= _parameters param [3, {}, [{}]];
	...
	case (toLower "setShotsData") : {
		...
		_object			= _parameters param [0, objNull, [objNull]];
		_shooterUid		= _parameters param [1, "", [""]];
		_shooterShotsData	= _parameters param [2, [], [[]]];
	...
	case (toLower "getShotTemplate") : {
		...
		_object 	= _parameters param [0, objNull, [objNull]];
		_shooter	= _parameters param [1, objNull, [objNull]];
		_bullet		= _parameters param [2, objNull, [objNull]];
		_position	= _parameters param [3, [], [[]]];
		_velocity	= _parameters param [4, [], [[]]];
		_selection	= _parameters param [5, [], [[]]];
		_ammo		= _parameters param [6, [], [[]]];
		_direction	= _parameters param [7, [], [[]]];
		_radius		= _parameters param [8, 0, [0]];
		_surface	= _parameters param [9, "", [""]];
		_direct		= _parameters param [10, false, [true]];
	...
	case (toLower "uiFillTable") : {
		...
		_object		= _parameters param [0, objNull, [objNull]];
		_listBox 	= _parameters param [1, controlNull, [controlNull]];
	...
	case (toLower "uiFillTableRow") : {
		...
		_object		= _parameters param [0, objNull, [objNull]];
		_listBox 	= _parameters param [1, controlNull, [controlNull]];
		_shooterArray	= _parameters param [2, [], [[]]];
		
		// The shooter data
		private ["_uid", "_name", "_shots"];
		_uid 	= _shooterArray param [0, "", [""]];
		_name	= _shooterArray param [1, "", [""]];
		_shots	= _shooterArray param [2, [], [[]]];
