...
_heli = _this param [1, objNull, [objNull]];
_showReports = _this param [2, true, [false]];

//WLIB removed - dependancy needs to be removed
//[] call BIS_fnc_initWlib;

// #include "\A3\functions_f\wlib\_shared\formatting.inc" - Commented out this file since it doesn't exist in SVN, and changed it's extension from hpp to inc since sqf files should not include hpps
...
if (_mode == 0) then
{
	if (!(_heli getVariable ["HSim_maintenanceMode", false])) then
	{
		...
		_hps = _this param [3, (_heli call BIS_fnc_helicopterGetHitpoints), [[]]];
		_displayNames = [];
		_damages = [];
		_inspections = [];
		_compartments = [];
