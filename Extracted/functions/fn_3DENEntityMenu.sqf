	...
	case "findcreate": {
		...
		switch (typename _entity) do {
			case (typename objnull): {
				...
				_name = "class " + configname _cfg;//gettext (_cfg >> "displayName");
		...
		if (_name != "") then {
			...
			_ctrlSearch = _display displayctrl IDC_DISPLAY3DEN_SEARCHCREATE;
		...
		_showTypes = {count _x > 0} count _entities > 1;
		{
			...
			if (count _x > 0) then {
				...
				};
				{
					...
					if (_name == "") then {
						_text = _text + _class;// + _br;
					} else {
						_text = _text + format ["%1 (%2)",_class,_name];// + _br;
					};
				} foreach _x;
				_n = _n + 1;
			};
