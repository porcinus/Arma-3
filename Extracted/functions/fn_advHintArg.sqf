...
_class = _this param [2,"undefined",[""]];
...
_j = 0;
{
	...
	if (_x isequaltype "") then {
		if (islocalized _x) then {
			_argArray pushback format ([localize _x] + BIS_advHintArg_argArrayPart1);
		} else {
			_res = call compile _x;
			if !(isnil "_res") then {
				_argArray pushback format ([_res] + BIS_advHintArg_argArrayPart1);
			};
		}
	} else {
		if (_x isequaltype []) then {
						...
						switch (typename _x) do {
							case (typename ""): {
								...
								if (count _actKey != 0) then {
									_actKeyForm = format ["<t color='#999999'>[<t color = '%2'>%1</t>]</t>",_actKey select 0,_keyColor];
									_argArray = _argArray + [_actKeyForm];
								} else {
									//error - wrong name of action or undefined key, actionKeysNamesArray return empty array
									_argArray = _argArray + [format ["<t color='#999999'>[<t color = '#FF0000'>%1: %2</t>]</t>",toUpper localize "STR_A3_Hints_unmapped",actionName _x]];
							...
							case (typename 00): {
								...
								_actKeyForm = format ["<t color='#999999'>[ <t color = '%2'>%1</t> ]</t>",_actKey,_keyColor];
								_argArray = _argArray + [_actKeyForm];
							};
							default {
