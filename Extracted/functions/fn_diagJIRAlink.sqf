private ["_type","_ctrl"];
_type = _this param [0,"",[""]];
_ctrl = _this param [1,controlnull,[controlnull]];
...
switch _type do {
	...
	case "object": {
		...
		if (isnull _target || count _cameras > 0) then {
			...
			_target = _targets param [0,objnull];
		};
...
if !(isnull ([] call bis_fnc_displayMission)) then {
	...
	_environment = format ["h3.+Image%1{color:grey}^!%2.png|thumbnail!^{color}%1%1",_br,_fileName] + _environment;
};
...
if (_fileName != "") then {
	_ctrl ctrladdeventhandler ["htmlLink",format ["screenshot ((profilenamespace getvariable ['BIS_fnc_diagJIRAlink_path','']) + '%1.png');",_fileName]];
};
_link