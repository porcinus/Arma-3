...
_alarm			= [_this, 5, objNull, [objNull]] call BIS_fnc_param;
...
if (_detected) then {
	...
	_detectedTrigger setTriggerActivation ["WEST", "GUER D", false];
	_detectedTrigger setTriggerStatements [
		format ["this && isNil { %1 getVariable 'BIS_cleared' }", _logic],
		format ["[%1] call BIS_fnc_b_m02_2_siteDetected;", _logic],
		""
	];
};
...
if (_cleared) then {
	...
	_clearedTrigger setTriggerActivation ["GUER", "NOT PRESENT", false];
	_clearedTrigger setTriggerStatements [
		"this",
		format ["[%1] call BIS_fnc_b_m02_2_siteCleared;", _logic],
		""
	];
};
