...
Example: 
["text",_logs,_chatbox,_local,_override] call BIS_fnc_NNS_debugOutput;
["text bal bla bla",true,true,true,false] call BIS_fnc_NNS_debugOutput;
...
_debugEnable = false;
if !(isNil "DebugOutputs_enable") then {_debugEnable = DebugOutputs_enable;}; //recover debug bool
if (_override) then {_debugEnable = true;}; //overide debug bool
...
if (!_debugEnable || {_text==""} || {(!_logs && !_chatbox)}) then {exit;}; //nothing to output, exit
...
if (_local) then { //local effect
	...
	if (_chatbox) then {[format["DEBUG (%1:%2) : %3",player,name player,_text]] remoteExec ["systemChat"];}; //global output to chatbox
