...
params [
["_spawnLocation", objNull],
["_allowedKind", ["Car", "Armored", "Tank", "Air"]]
];
...
if (count _allowedKind == 0) then {_allowedKind = ["Car", "Tank"]};

_spawnWasObject = false;
_spawnDir = 0;
if !(_spawnLocation isEqualType []) then {_spawnWasObject = true; _spawnDir = getDir _spawnLocation; _spawnLocation = getPos _spawnLocation}; //_spawnLocation is a object
...
_classToUse = selectRandom _allowedClasses;
_tmpVehi = createVehicle [configName _classToUse, [0,0,0], [], 0, "CAN_COLLIDE"];
_tmpVehi setDir _spawnDir;
_tmpVehi setPos _spawnLocation;

systemChat format ["God granted you a %1 (%2)", [_classToUse] call BIS_fnc_displayName, configName _classToUse];
