...
Example: 
_null = [[500,500],270] call BIS_fnc_NNS_spawnCivVehi;
...
// Check for validity
if (count _pos < 2) exitWith {["BIS_fnc_NNS_spawnCivVehi : invalid position"] call BIS_fnc_NNS_debugOutput;};

if (count _civClasses == 0) then {_civClasses = missionNamespace getVariable ["BIS_civilCars",[]];};
if (count _civClasses == 0) exitWith {["BIS_fnc_NNS_spawnCivVehi : no _civClasses set"] call BIS_fnc_NNS_debugOutput;};

if (count _civWreckClasses == 0) then {_civWreckClasses = missionNamespace getVariable ["BIS_civilWreckCars",[]];};
if (count _civWreckClasses == 0) then {_wreckChance = 0; ["BIS_fnc_NNS_spawnCivVehi : no _civWreckClasses set, wreck change set to 0"] call BIS_fnc_NNS_debugOutput;};
...
//if (count _alarmClasses == 0) then {_wreckChance = 0; ["BIS_fnc_NNS_spawnCivVehi : no _alarmClasses set, alarm change set to 0"] call BIS_fnc_NNS_debugOutput;};
...
if (_wreck) then {
	...
	if (_allowDamage) then {[_veh,["hitfuel"],0.2,0.8] call BIS_fnc_NNS_randomVehicleDamage;};
};
...
if (_alarm && {!(_wreck)}) then {
	...
	[_pos,_veh,_newGrp,_alarmMaxDist] spawn {
		...
		if !(isNull _grp) then {
			...
			[format["'BIS_fnc_NNS_spawnCivVehi' group:%1 killed because too far from player",_grp]] call BIS_fnc_NNS_debugOutput; //debug
		};
	};
};
