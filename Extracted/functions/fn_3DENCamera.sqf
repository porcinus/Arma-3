...
switch _mode do {
	case "move": {
		...
		if !(_pos isequaltypearray [0,0,0]) exitwith {};

		//--- Move camera
		_camera = get3DENCamera;
		_cameraPos = _pos getpos [50,getdir _camera + 180];
		_cameraPos set [2,getterrainheightASL _pos + 50];
		_camera setposasl _cameraPos;
		[_camera,-45,0] call bis_fnc_setpitchbank;

		//--- Move map
		_ctrlMap = (finddisplay IDD_DISPLAY3DEN) displayctrl IDC_DISPLAY3DEN_MAP;
		_ctrlMap ctrlmapanimadd [0.2,ctrlmapscale _ctrlMap,_pos];
