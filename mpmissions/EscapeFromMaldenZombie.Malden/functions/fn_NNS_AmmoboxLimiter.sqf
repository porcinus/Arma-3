...
Example: _null = [ammobox] call BIS_fnc_NNS_AmmoboxLimiter;
Example: _null = [ammobox,0.1, 0.75] call BIS_fnc_NNS_AmmoboxLimiter;
*/
...
// Check for validity
if (isNull _box) exitWith {[format["BIS_fnc_NNS_AmmoboxLimiter : Non-existing ammo box %1 used!",_box]] call BIS_fnc_NNS_debugOutput;};
...
if (_max <= 0) exitWith {["BIS_fnc_NNS_AmmoboxLimiter : Max value <= 0"] call BIS_fnc_NNS_debugOutput;};
if (_min >= 1) exitWith {["BIS_fnc_NNS_AmmoboxLimiter : Min value >= 1"] call BIS_fnc_NNS_debugOutput;};
...
if (_itemsCount == 0 && _magsCount == 0) exitWith {["BIS_fnc_NNS_AmmoboxLimiter : Ammobox contain nothing"] call BIS_fnc_NNS_debugOutput;};
