...

{
	if !(_x in _ignore) then {
		...
		//[format["NNS_fnc_randomVehicleDamage : %1 : %2 : %3",_vehicle,_x,_random]] call NNS_fnc_debugOutput; //debug
	}else{
		//[format["NNS_fnc_randomVehicleDamage : %1 : %2 ignored",_vehicle,_x]] call NNS_fnc_debugOutput; //debug
	};
