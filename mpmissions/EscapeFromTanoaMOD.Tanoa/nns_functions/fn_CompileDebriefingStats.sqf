...
Example: 
_null = [false] call NNS_fnc_CompileDebriefingStats; //run on server, will automatically remote exec on clients
*/
...
if (_client_exec) then { //NNS : stats : convert to global when mission end
	...
	["NNS_fnc_CompileDebriefingStats : Stats variables set to global"] call NNS_fnc_debugOutput; //debug
};

if (isServer && {!_client_exec}) then { //if server, compile all data, TODO: to localize
	[true] remoteExec ["NNS_fnc_CompileDebriefingStats",0,true]; //ask everyone to set stats variable to global
	sleep 5; //allow some time for everyone
	
	["NNS_fnc_CompileDebriefingStats : Server start compile players stats"] call NNS_fnc_debugOutput; //debug
