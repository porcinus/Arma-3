...

{
	...
	switch (tolower _role) do
	{
		...
		case "turret":
		{
			if (isnull (_center turretunit _index) && _index in allturrets _center) then
			{
				_unit = call _fnc_getUnit;
		...
		case "cargo":
		{
			if (!(_index in _occuppiedCargo) && _center emptypositions "cargo" > 0) then
			{
				_unit = call _fnc_getUnit;
