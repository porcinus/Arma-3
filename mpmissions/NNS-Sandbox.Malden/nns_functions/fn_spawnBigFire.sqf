...
Example: 
_null = [vehi01] call NNS_fnc_spawnBigFire;
...
// Check for validity
if (isNull _object) exitWith {[format["NNS_fnc_spawnBigFire : Non-existing object %1 used!",_object]] call NNS_fnc_debugOutput;};
...
if (((time - _startTime) > 600) && ((player distance2d _pos) > (_deletionRadius * 0.9))) exitWith {["NNS_fnc_spawnBigFire : Failed : 10 minutes timeout"] call NNS_fnc_debugOutput;};
