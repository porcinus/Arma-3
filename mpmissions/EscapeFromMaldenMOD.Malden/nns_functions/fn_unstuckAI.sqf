...
// Check for validity
if (isNull _target) exitWith {[format["NNS_fnc_unstuckAI : Non-existing unit %1 used!",_target]] call NNS_fnc_debugOutput;};

_running = true;

_group = grpNull;
if (typeName _target != "GROUP") then {_group = group _target;} else {_group = _target;}; //detect if group or unit
[format["NNS_fnc_unstuckAI : %1 : Starting stuck detection",_group]] call NNS_fnc_debugOutput; //debug
	...
	sleep (_interval);
	{
		...
		if (alive _unit) then { //alive unit
			...
			if ((_old_pos distance _new_pos) <= _threshold) then { //unit didn't moved enough in given interval
				[format["NNS_fnc_unstuckAI : %1 : %2 look to be stuck",_group,_unit]] call NNS_fnc_debugOutput; //debug
				...
				if !(isNull objectParent _unit) then { //unit is in a vehicle
					...
					[format["NNS_fnc_unstuckAI : %1 : Vehicle (%2) teleported at %3m",_group,_unit,_unit distance (getPosATL _unit)]] call NNS_fnc_debugOutput; //debug
				} else {
					_unit setPosATL [(_new_pos select 0) + (random _distance),(_new_pos select 1) + (random _distance),_new_pos select 2]; //teleport unit
					[format["NNS_fnc_unstuckAI : %1 : Unit (%2) teleported at %3m",_group,_unit,_unit distance (getPosATL _unit)]] call NNS_fnc_debugOutput; //debug
				};
				_running = false;
			};
		};
...
if(count _new_waypoint_set > 0) then { //set of waypoint given
...

	{
		...
		[format["NNS_fnc_unstuckAI : %1 : Waypoint added : %2",_group,_x]] call NNS_fnc_debugOutput; //debug
...
if(_alive_units > 0 && {_alive_vehicle} && {_restart}) then {_null = [_target,_interval,_threshold,_new_waypoint_set,_distance,_restart] call NNS_fnc_unstuckAI;}; //restart if set

[format["NNS_fnc_unstuckAI : %1 : Detection end",_group]] call NNS_fnc_debugOutput; //debug
