...
switch (true) do
{
	...
	case (abs (_r1 - _r2) >= _d) :
	{
		...
		if(_r1 > _r2) then
		{	
			_position = [ [(_positions select 1) select 0, (_positions select 1) select 1, 0] , _r2] call BIS_fnc_randomPosTrigger;
		}
		else
		{
			_position = [ [(_positions select 0) select 0, (_positions select 0) select 1, 0] , _r1] call BIS_fnc_randomPosTrigger;
