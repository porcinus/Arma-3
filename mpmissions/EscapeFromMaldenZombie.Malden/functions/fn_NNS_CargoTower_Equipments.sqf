...
Example: _null = Test_Tower_1 call BIS_fnc_NNS_CargoTower_Equipments;
...
// Check for validity
if (isNull _tower) exitWith {[format["BIS_fnc_NNS_CargoTower_Equipments : Non-existing unit tower %1 used!",_tower]] call BIS_fnc_NNS_debugOutput;};

[format["BIS_fnc_NNS_CargoTower_Equipments : %1m",player distance2d _tower]] call BIS_fnc_NNS_debugOutput; //debug

if ((damage _tower) > 0.99) exitWith {[format["BIS_fnc_NNS_CargoTower_Equipments : Tower too damaged to add equipment (%1)",damage _tower]] call BIS_fnc_NNS_debugOutput;};
