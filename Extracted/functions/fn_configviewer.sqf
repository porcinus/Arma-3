...
switch _mode do {
	...
	case "Init": {
		...
		if (str BIS_fnc_configviewer_previewCode != str {}) then {
			...
			GUI_PREVIEWCLASS ctrlsettooltip localize "str_a3_rscdisplayconfigviewer_buttonpreviewclass_tooltip";
			GUI_PREVIEWPARAM ctrlsettooltip localize "str_a3_rscdisplayconfigviewer_buttonpreviewparam_tooltip";
	...
	case "keyDown": {
		...
		if (_ctrl) then {
				...
				case DIK_C: {
					if (_isClasses) then {
						if (_shift) then {
							...
							for "_i" from 0 to (lbsize GUI_CLASSES - 1) do {
								if ((GUI_CLASSES lbvalue _i) == _value) then {
									_text = _text + (GUI_CLASSES lbdata _i) + _br;
								};
							};
			...
			//--- No Ctrl
			if (_key in [DIK_RETURN,DIK_NUMPADENTER,DIK_BACK]  && (count BIS_fnc_configviewer_keyList isEqualTo 0) ) then {
				...
				_alphabetDik = [DIK_SPACE,DIK_A,DIK_B,DIK_C,DIK_D,DIK_E,DIK_F,DIK_G,DIK_H,DIK_I,DIK_J,DIK_K,DIK_L,DIK_M,DIK_N,DIK_O,DIK_P,DIK_Q,DIK_R,DIK_S,DIK_T,DIK_U,DIK_V,DIK_W,DIK_X,DIK_Y,DIK_Z,DIK_MINUS,DIK_1,DIK_2,DIK_3,DIK_4,DIK_5,DIK_6,DIK_7,DIK_8,DIK_9,DIK_0];
				...
				if (_keyId >= 0) then  {
					...
					[GUI_CLASSES,_isClasses] spawn {
						...
						with uinamespace do {
							...
							endloadingscreen;
							// delayed clearing of search box
							uisleep 5; //--- Originally 0.3
							if (count BIS_fnc_configviewer_keyList != _keyCount) exitwith {};
					...
					if(_key in [DIK_BACK,DIK_DELETE])then
					{
						_textSearch = if (_isClasses) then {GUI_CLASSES_SEARCH} else {GUI_PARAMS_SEARCH};
